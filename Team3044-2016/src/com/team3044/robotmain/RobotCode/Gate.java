...
public class Gate {
	...
	public enum state {
		Init, encoderZeroing, Stopped, movingUp, movingDown, ballPickUpState
	}
	...
	public void gateTeleopPeriodic() {
		...
		case movingDown:
			...
			break;
		case ballPickUpState:
			if(!CommonArea.pickUpBoulder){
				Components.getInstance().gateTalon.set(0);
				gateState = state.Stopped;
			} else if(Components.getInstance().gateTalon.getEncPosition() > 1325){
				Components.getInstance().gateTalon.set(motorSpeedUp);
			} else if(Components.getInstance().gateTalon.getEncPosition() < 1100){
				Components.getInstance().gateTalon.set(motorSpeedDown);
			} else {
				Components.getInstance().gateTalon.set(0);
			}
	...
	public void gateTestPeriodic() {
		...
		/*
		 * if(controller.getRawButton(3)){ if (comp.BallInLimit.get()){
		 * comp.shooterTrack.set(0); }else{ comp.shooterTrack.set(-1); } }else
		 * if (controller.getRawButton(2)){ comp.shooterTrack.set(1);
		 * 
		 * }else if (controller.getRawButton(6)){ comp.shooterTrack.set(-1);
		 * }else{ comp.shooterTrack.set(0); }
		 * 
		 * if(controller.getRawButton(4)){ if(comp.gateTalon.getEncPosition() >
		 * -500){ comp.gateTalon.set(-.3); }else{ comp.gateTalon.set(0); } }else
		 * if (controller.getRawButton(1)){ if(comp.gateTalon.getEncPosition() <
		 * 2500 ){ comp.gateTalon.set(.4);; }else{ comp.gateTalon.set(0); }
		 * 
		 * } else{ comp.gateTalon.set(0);
		 * 
		 * }
		 * 
		 * 
		 * //Components comp = Components.getInstance();
		 * //comp.gateTalon.set(SmartDashboard // .getDouble("DB/Slider 0"));
		 * SmartDashboard.putString("DB/String 5",
		 * String.valueOf(comp.gateTalon.getEncPosition()));
		 * SmartDashboard.putString("DB/String 6",
		 * String.valueOf(comp.BallInLimit.get()));
		 * 
		 * //SmartDashboard.putString("Current "+"DB/String 0",
		 * String.valueOf(comp.gateTalon.getOutputCurrent()));
		 * //SmartDashboard.putString("Encoder "+ "DB/String 0 ",
		 * String.valueOf(comp.gateTalon.getEncPosition()));
		 * 
		 * //SmartDasDashboard.putString("Gate Down limit " + "DB/String 2",
		 * String.valueOf(comp.GateDownLimit.get()));
		 * //SmartDashboard.putString("Motor Speed" + "DB/String 3",
		 * String.valueOf(controller.getRightY()));
		 */
	}
}
